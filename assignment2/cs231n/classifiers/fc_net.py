...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        inp = X
        cache, gamma, beta = None, None, None
        caches = {}
        bn_param = None
        for i in range(self.num_layers - 1):
        ...
        dout, dW, db = affine_backward(dout, caches['{}'.format(self.num_layers)])
        grads['W{}'.format(self.num_layers)] = dW + self.reg*self.params['W{}'.format(self.num_layers)]
        grads['b{}'.format(self.num_layers)] = db
        reg_loss += np.sum(np.square(self.params['W{}'.format(self.num_layers)]))

        for i in reversed(range(self.num_layers - 1)):
            dout, dW, db, dgamma, dbeta = affine_norm_relu_backward(dout, caches['{}'.format(i + 1)],
            ...
            reg_loss += np.sum(np.square(self.params['W{}'.format(i + 1)]))
