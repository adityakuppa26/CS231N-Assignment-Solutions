...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x_hat, gamma, ivar, xmu, sqrtvar, var, eps, layernorm)
...
def batchnorm_backward(dout, cache):
    ...
    x_hat, gamma, ivar, xmu, sqrtvar, var, eps, axis = cache
    N, D = dout.shape
    ...
    dx1 = dxmu1 + dxmu2
    dmean = -1 * np.sum(dxmu1+dxmu2, axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    x_hat, gamma, _, _, _, var, _, axis = cache
    dgamma = np.sum(dout * x_hat, axis=axis)
    dbeta = np.sum(dout, axis=axis)
    dx_hat = dout * gamma
    dx = dx_hat - np.sum(dx_hat, axis=0)/N - np.sum(dx_hat*x_hat, axis=0)*x_hat/N
    dx /= np.sqrt(var)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for f in range(F):
            for h in range(outH):
                for wid in range(outW):
                    out[n,f,h,wid] = np.sum(w[f, :, :, :] * x_pad[n, :, h*stride:h*stride+FH, wid*stride:wid*stride+FW])+b[f]
    ...
    for n in range(N):
      for f in range(F):
        for h in range(outH):
          for wid in range(outW):
            db[f] += dout[n, f, h, wid]
            dx_pad[n, :, h*stride:h*stride+FH, wid*stride:wid*stride+FW] += w[f, :, :, :]*dout[n, f, h, wid] 
            dw[f, :, :, :] += x_pad[n, :, h*stride:h*stride+FH, wid*stride:wid*stride+FW]*dout[n, f, h, wid]
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
      for c in range(C):
        for h in range(outH):
          for wid in range(outW):
            idx = np.argmax(x[n, c, h*stride:h*stride+pool_height, wid*stride:wid*stride+pool_width])
            idx1, idx2 = np.unravel_index(idx, (pool_height, pool_width))
            dx[n, c, h*stride:h*stride+pool_height, wid*stride:wid*stride+pool_width][idx1, idx2] += dout[n, c, h, wid]
